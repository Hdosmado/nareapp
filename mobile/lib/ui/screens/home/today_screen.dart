...
class _Content extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Mantiene activo el planificador de tracking automático SIN mostrar nada
    // en pantalla: la ubicación se informa solo por la notificación del SO, no
    // con un cartel in-app (eso solo invitaría a querer desactivarla).
    ref.watch(trackingControllerProvider);
    // Solo avisamos cuando NO hay conexión (no cuando sincroniza): el aviso de
    // "sin conexión" es útil; el de "sincronizando" parpadeaba con cada latido.
    // `select` evita reconstruir salvo que cambie el estado online/offline.
    final offline =
        !ref.watch(syncControllerProvider.select((s) => s.isOnline));
                ...
                if (offline)
                  Padding(
                    ...
                    child: NareBanner(
                      ...
                      title: 'Sin conexión',
                      body: 'Guardamos tus registros y los enviamos solos '
